...
def tune(device_name):
    ...
    params = { "filter_width": filter_width, "filter_height": filter_height, "block_size_x": 16, "block_size_y": 16 }
    results = kernel_tuner.run_kernel("convolution_naive", "convolution.cu",
    ...
    #start tuning
    results, env = kernel_tuner.tune_kernel("convolution_kernel", "convolution.cu",
        problem_size, args, tune_params,
        grid_div_y=grid_div_y, grid_div_x=grid_div_x, cmem_args=cmem_args,
        verbose=True, restrictions=restrict, cache="convolution_" + device_name,
        metrics=metrics, iterations=32, device=0)
