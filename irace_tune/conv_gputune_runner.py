...
if __name__=='__main__':
    ...
    while cand_params:
        ...
        if param == "--block_size_x":
            bx = int(value)
            conv_params.append((param, bx))
        elif param == "--block_size_y":
            by = int(value)
            conv_params.append((param, by))
        elif param == "--tile_size_x":
            tx = int(value)
            conv_params.append((param, tx))
        elif param == "--tile_size_y":
            ty = int(value)
            conv_params.append((param, ty))
        elif param == "--use_padding":
            up = int(value)
            conv_params.append((param, up))
        elif param == "--read_only":
            ro = int(value)
            conv_params.append((param, ro))
        else:
            target_runner_error("unknown parameter %s" % (param))
    #print("\niRace parsed the param values:", conv_params)

    # Sanity checks
    if None in [bx, by, tx, ty, up, ro]:
