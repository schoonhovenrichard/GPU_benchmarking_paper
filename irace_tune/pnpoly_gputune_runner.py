...
if __name__=='__main__':
    ...
    while cand_params:
        ...
        if param == "--block_size_x":
            block_size_x = int(value)
            pnpoly_params.append((param, block_size_x))
        elif param == "--tile_size":
            tile_size = int(value)
            pnpoly_params.append((param, tile_size))
        elif param == "--between_method":
            between_method = int(value)
            pnpoly_params.append((param, between_method))
        elif param == "--use_precomputed_slopes":
            use_precomputed_slopes = int(value)
            pnpoly_params.append((param, use_precomputed_slopes))
        elif param == "--use_method":
            use_method = int(value)
            pnpoly_params.append((param, use_method))
        else:
            target_runner_error("unknown parameter %s" % (param))
    #print("\niRace parsed the param values:", pnpoly_params)

    # Sanity checks
    if None in pnpoly_params:
    ...
    fitness = irace_gpu_reader.return_GPU_score(pnpoly_params)
    if fitness > 1000000:
        print(str('inf') + '\n')
        sys.exit(0)
    else:
